...
This chapter will give a short introduction to the work of SSRS, the research field of QoE as well as the scope of thesis project. 
...
As part of the project a mission control software in which one can manage a fleet of drones and see their video feeds. However, the drone camera can only be controlled by a so called region of interest (ROI), which points the camera at a GPS location provided on a map. This is not ideal for the operator, as the act of uploading a new ROI requires a point to be selected on the map and then uploaded to the drone, forcing a context switch for the operator from the video feed to the map. Instead, SSRS wants to be able to directly adjust the gimbal angle with direct controls.
...
\section{Drone use}
With the recent advancements in technology, the use of drones has skyrocketed. Most notably they have been used in warfare for surveillance and attack purposes, but in recent years the civilian applications have increased (SOURCES). Common civilian use cases are in photography, agriculture and delivery, and there is also a growing interest in using drones for search and rescue (GET MORE EXAMPLES).  
