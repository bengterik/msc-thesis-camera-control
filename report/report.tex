...
\chapter{Conclusion}
In this chapter the main takeaways from the two experiments will be summarized.

\section{QoE Experiment}
From the quantative results a clear trend can be seen that the delay has a negative effect on the performance of the subjects. The data also indicate that the negative effect on the subject's performance was bigger between 400ms and 600ms than between 0ms and 200ms of added delay. This suggests that the effects of delay might not be linear after a certain threshold.

According to the forms answered after each trial, the perception of the system got worse as more delay was added. The subjects reported that their sence of control decreased dramatically with more added delay, however, their rating on task performance remained equal for delays 200ms, 300ms and 400ms. Their impression of the system also got worse with more delay, although the impression for delays 200ms and 300ms remained equal.

As was the intention with having both quantative and qualitative measurements, it can be seen that the experience of a user is not always proportional to the worsening of QoS parameters.    

The subjects experience a very small increase in symptoms during the trials, but the increase is not large enough to be considered significant for a sample size of only 10 subjects.
