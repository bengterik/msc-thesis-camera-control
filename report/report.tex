...
To measure each subjects performance, a script using a detection algorithm was used on the recorded video which for each frame gave the distance between the red dot and the center of the apriltag. The formula for calculating the pixel distance between the two points is shown in equation \ref{eq:distance}. A frame from the detection script can be seen in figure \ref{fig:resultcalc}.
...
As a proof of concept, an auto-follow mode was developed which allowed the gimbal to follow the object. This was made using a PI-controller based on inputs from the object detection in real-time. Due to time limitations this was not investigated any further.
