...
As fixed-wing UAVs have seen heavy use in the military sector, a high level of sophistication in the UAVs as well as their operation is usually found in these types of systems. One of the most historically known military drones are the MQ-9 Reaper series \cite{mq9reaper}, which saw heavy use by the U.S. during the conflicts in Iraq and Afghanistan \cite{mq9reaper-wiki}. More modern systems include the Turkish Bayraktar drones \cite{bayraktar-wiki} as well as the Switchblade series from the American company Aerovirement \cite{switchblade}.

In the consumer market, the chinese manufacturer DJI holds a global market share of more than 70\%, according to CNBC \cite{dji}. Although they mainly focus on rotor drones, their most popular systems are targeted towards filming and thus feature camera gimbals. These usually have gimbal modes such as stabilization, target following and ROI-mode. 

There area also manufacturers specialized in drone gimbals for companies or people building there own drones, such as Gremsy \cite{gremsy} and SIYI \cite{siyi}. These gimbals cost \$1,399 and \$529.42 dollars respectively and their functionality is highly dependant on the on on board software. For comparison, the SSRS gimbal can be assembled for (EXAKT PRIS?).
...
\section{Impact}
The test bed developed in this thesis shows promise as a tool for evaluating different parameters relevant to QoE research. Something that sticks out in the test bed is also the ability to get a lot of data on quantitative performance through the use of image recognition.
