...
\theabstract{
This thesis work forms part of a project aiming to investigate the use of drones in maritime search-and-rescue (SAR) operations The project is organized by the Swedish Sea Rescue Society (SSRS) and thus far in the project, a fixed-wing drone along with mission control software has been developed. In this thesis work, the focus has been to improve the camera-controls of the drone by implementing a gimbal control interface to evaluate the suitability of manually controlling the camera. 

To evaluate the interface, two experiments were conducted. The first in a lab setting, studying the effects of latency on an operator. The second, integrating the software into the existing systems and evaluating the controls when in the air. 

The results from the first experiment show that increasing latency worsens the subject's task performance as well as their subjective experience with the system. Comparing the rates at which they worsen, the data suggests that task performance decreases exponentially while the subjective experience decreases linearly, although more trials are required to establish their exact relationship. In the second experiment, the software was successfully integrated with the pre-existing systems, and a test flight was made over the Gothenburg archipelago. During the flight, the manual controls were deemed most suitable when flying straight for longer periods, either for surveying or following a moving object.

In conclusion, a versatile test bed was developed which could be used both to conduct a laboratory experiment as well as to control the camera of the drone in flight. 
}
