...
\begin{description}
   ...
   \item[MAVProxy] Each message must be taken from the ethernet interface of the Raspberry Pi and be sent on the serial interface to the flight controller. 
   
   \item[Flight Controller] The flight controller must process the message and send the appropriate PWM signals to the gimbal motors. The processor running the firmware is a small chip and it is possible that gimbal control is not a high priority message in the software.   
\end{description}
...
The weather was sunny with almost no clouds. The wind was estimated to be around 5-9 m/s at ground level and the temperature was around 12 degrees Celsius.
