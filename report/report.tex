...
The desire was to create a task that would be as close to the real world application as possible while still making it easy to measure the performance of the operator. The system in the real world is very dynamic, so some sort of movement was desired. As the hoverboard was accessible in the lab and could be controlled easily, it was chosen as the moving platform. At first the desire was to have the gimbal box on top of the drone, having the drone move in a circle in an upside-down loiter. This was deemed to introduce several unknowns into the test bed, as the drone would have to be connected wirelessly to power and the operator in order to not have to run over any cables with the hoverboard. The lighting from the ceiling as well as the windows also reduced the contrast of the apriltag, making it harder to detect for the algorithm.
