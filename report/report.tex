...
\section{Limitations}
The main limitation of the test bed is that the scenario keeps the drone stationary, which is not the case in real flight. Although the results from the experiment are interesting in their own right, they are not directly applicable to the SSRS scenario. Another limitation with the test bed is the fact that the hoverboard ran in a predictable path. Because of this, the subject's ability to predict the path could have had an effect on their performance, which this study does not take into account.

In a real flight scenario, conducting controlled experiments faced significant limitations due to various factors. The primary obstacle lies in the extensive preparations required for each mission. Setting up the drone and its intricate systems demands substantial effort and time. Furthermore, numerous regulations and safety measures must be adhered to before undertaking an unmanned BVLOS flight. Additionally, communication with air traffic control about the mission is initialized two weeks before the flight, and contact is held during the entirety of the mission. Due to the scarcity of actual flight tests, gimbal control was not the only goal of the flight, which meant that there were limited possibilities to orchestrate a good scenario to evaluate operator performance in the system during flight. 

Another feature that was not tested was the so called "earth-frame", which is a gimbal-control-mode which points the gimbal in a compass heading, effectively stabilizing the yaw-axis as well. This mode was not tested in the experiment, but could be of interest in further implementations.
