...
\theabstract{
This thesis work forms part of a project called Eyes-On-Scene (EOS) organized by the Swedish Sea Rescue Society (SSRS), where a fixed-wing drone has been developed to investigate the use of drones in maritime search-and-rescue (SAR) operations. In the thesis work, the focus has been to improve the camera-controls of the drone, and a gimbal control interface has been developed to evaluate the suitability of manually controlling the camera. 

To evaluate the interface, two experiments were conducted. The first in a lab setting, studying the effects of latency on an operator. The second, integrating the software into the current EOS system and evaluating the controls when in the air. 

The results from the first experiment show that increasing latency worsens the subject's task performance as well as their subjective experience with the system. Comparing the rates at which they worsen, the data suggests that task performance decreases exponentially while the subjective experience decreases linearly, although more trials are required to establish the exact relationship they have with latency. In the second experiment, the software was successfully integrated with the existing systems, and a test flight was made over the Gothenburg archipelago. During the flight, the manual controls were deemed most suitable for surveying and following a moving object when the drone was flying in a straight line.

In conclusion, a versatile test bed was developed which could be used both to conduct a laboratory experiment as well as to control the camera in flight. 
}
