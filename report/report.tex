...
Although the lab environment in which the experiment is conducted is a comparatively controlled environment, a natural consequence during the course of the project has been to evaluate the strengths and weaknesses of different system architectures. This section will bring up some of the different requierments along with the particular architectures considered for the application of sea rescue.

Being a real-time application over the Internet, there are many parts of the system that can induce latency and efforts to reduce the latency can be boiled down to two actions: 
\begin{itemize}
   \item reducing the amount of processing agents between the operator and what is being controlled
   \item reducing the amount of processing at the processing agents
\end{itemize} 
However, while one particular architecture might meet requierments in one area, for example latency, it can make other parts of the system more complex or computationally expensive. In order to balance the different tradeoffs the specific application in has to be taken into account. 

\section{WebRTC and data channels}
The use of WebRTC along with Janus enables two units to talk to one another directly over the web, removing a potential server from the  in order to reduce latency one could see that it would be benificial 
