...
\acknowledgements{
I would like to extend my sincere thanks to Fredrik Falkman for introducing me to the EOS-project and for providing the necessary hardware resources enabling the thesis work. I am also grateful to my supervisor, William Tärneberg, along with Haorui Peng for their many ideas and technical advice. Lastly, I want to thank Oscar Sanner for allowing me to use his thesis work as a part of my test bed and for letting me bounce many ideas throughout the course project.
}

\theabstract{
This thesis work forms part of a project investigating the use of drones in maritime search-and-rescue (SAR) operations, where they are to provide early imagery of a maritime incident to assist the decision making of rescue personnel on their way to the scene. The focus of the work is on controlling a camera gimbal on a fixed-wing drone, and in the thesis a gimbal control interface has been developed with the aim of acheiving manual control of the camera while also studying the effects of network latency on the operator's experience. Two experiments have been carried out: one user study, evaluating quantative and qualitative effects of network latency on a camera operator, and one field test, where the gimbal control software was tested during a flight over the Gothenburg archipelago.

In the user study, the user was given a task to perform provided a joystick and a video feed from the drone camera. The task was completed five times with different latencies introduced in the program. The results indicate that the subjects performance worsened at a nonlinear rate with more added latency, and that the subject's performance and their experience did not decrease with the same rate.

In the second experiment, the software was successfully integrated with the in-flight systems of the drone. A test flight was made over the Gothenburg Archipelago and the manual controls were deemed suitable for surverying and following an object when the drone was flying in a straight line.  
}
...
Addressing goal \textbf{G1} of the thesis, a program was implemented in Python displaying the video feed from the drone camera while taking control inputs from the operator. The inputs came from the right joystick of a PlayStation-controller while the program sent messages updating the gimbal's position with a set frequency over IP. The program could also record the video feed displayed to the gimbal operator. A visual overview of the system is shown in Figure \ref{fig:system-overview}.
\begin{figure}[!hbt]
   \centering
