...
\theabstract{
The objective of this thesis has been to improve the camera-controls of a fixed-wing drone to be used in maritime search-and-rescue (SAR) operations by the Swedish Sea Rescue Society (SSRS). A gimbal control interface has been implemented that displays the video feed of the drone camera while taking control inputs from a joystick that moves its view in real-time. 

The interface has then been used to conduct two experiments. The first in a lab setting, studying the effects of latency on the camera operator given a specific task. The results from this experiment show that increasing latency worsens both the operator's ability to perform the task as well as their subjective experience with the system. Comparing the rates at which they worsen, the data suggests that task performance decreases exponentially while the subjective experience decreases linearly, although more trials are required to establish the exact relationships. 

In the second experiment, the software was integrated with the pre-existing systems of SSRS, and a test flight using the manual controls was made over the Gothenburg archipelago. During the flight, the manual controls were deemed most suitable when flying straight for longer periods, either for surveying or following a moving object.

In conclusion, a versatile test bed allowing for manual control of the drone's camera was developed. For the SSRS, it will serve as a starting point for better camera controls, while it also can be used in further research to understand the relationship between network parameters and an operator's quality of experience.
}
